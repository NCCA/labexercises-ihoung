...
void Emitter::resetParticle(size_t _index)
{
    if(Random::randomPositiveFloat(500) > 450)
    {
        ...
        m_color[_index] = Random::randomPositiveVec3();
    ...
    else
    {
        ...
        m_posSize[_index].set(-1000,-10000,-10000,0.0);
...
void Emitter::render() const
{
    ...
    //     ngl::ShaderLib::setUniform("colour", p.color);
...
void Emitter::saveFrame(int _frameNo) const
{
    ...
    //     ss << m_color[i].m_x << ' ' << m_color[i].m_y << ' ' << m_color[i].m_z << ' ';
    //     ss << m_size[i] << ")\n";
    // }
    // ss << "PrimitiveAttrib \n";
